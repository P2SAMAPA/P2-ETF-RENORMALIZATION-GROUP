...
st.set_page_config(page_title="Renormalization Group Flow", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🌀 Renormalization Group Flow</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Wilsonian RG | Coarse‑graining | Fixed points | Scale‑invariant regimes</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 RG Flow")
...
st.sidebar.markdown(f"**Block sizes:** {', '.join(map(str, config.BLOCK_SIZES))}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'rg_flow_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>RG stability: {etf['rg_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized RG Score"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized RG Score", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🌀 Top ETFs by RG Stability (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Wilsonian Renormalization Group** coarse-grains the time series at different scales.
        - Couplings (mean, volatility, skewness, kurtosis) flow under RG transformations.
        - **Fixed points** are scale‑invariant regimes (stable market states).
        - The score = 1 / (1 + distance from current couplings to fixed point).
        - High score → ETF is near a fixed point → stable regime.
        - Low score → ETF is far from fixed point → likely regime change.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the RG stability scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Renormalization Group | Wilsonian RG for scale‑invariant regime detection")
